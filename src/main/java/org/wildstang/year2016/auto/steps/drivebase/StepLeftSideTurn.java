 ...
 * @author Joey
 */
public class StepLeftSideTurn extends AutoStep {

    private double value, angle;
    private boolean shouldFinish = false;

    public StepLeftSideTurn(double relativeAngle) {
        ...
        if (value < 0) {
            ...
            if (angle < gyroAngle) {
                ((DriveBase) Core.getSubsystemManager()
                        .getSubsystem(WSSubsystems.DRIVE_BASE.getName())).setLeftDrive(0.0);
